...
class FEATHER:
    ...
    def _create_D_inverse(self, graph):
        ...
        values = np.array([1.0/graph.degree[node]
...
class FEATHERG:
    ...
    def _fit_a_FEATHER(self, graph):
        ...
        feature = np.array([math.log(graph.degree(node)+1)
                            for node in range(graph.number_of_nodes())])
    ...
    def fit(self, graph_paths):
        ...
        """
        self._X = []
        for path in tqdm(glob(graph_paths + '*')):
            print(path)
            edges = json.load(open(path))['edges']
            G = nx.from_edgelist(edges)
            self._X.append(self._fit_a_FEATHER(G))
